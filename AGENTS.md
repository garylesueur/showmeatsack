...
- Shared brand files (characters, voice, tokens, homepage shell) live in the `brand/` submodule ([meatsack-brand](https://github.com/garylesueur/meatsack-brand)). Run `pnpm sync:brand` after updating it. Never edit the generated copies; `pnpm test` fails if they drift.
